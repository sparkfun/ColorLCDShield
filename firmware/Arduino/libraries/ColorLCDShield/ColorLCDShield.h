...
#ifndef LCD_USING_SPI
#define LCD_USING_SPI 0
#endif
...
#ifndef HAVE_PWM_BRIGHTNESS_CONTROL
#define HAVE_PWM_BRIGHTNESS_CONTROL 0
#endif
...
#define BLUE		0x000F
#define TEAL 		0x0088
#define EMERALD		0x00C5
#define	GREEN		0x00F0
#define CYAN		0x00FF
#define SLATE 		0x0244
#define INDIGO  	0x0408
#define TURQUOISE	0x04ED
#define OLIVE 		0x0682
#define MAROON 		0x0800
#define PURPLE 		0x0808
#define GRAY 		0x0888
#define SKYBLUE		0x08CE
#define BROWN		0x0B22
#define CRIMSON 	0x0D13
#define ORCHID 		0x0D7D
#define RED		0x0F00
#define MAGENTA		0x0F0F
#define ORANGE 		0x0F40
#define PINK		0x0F6A
#define CORAL 		0x0F75
#define SALMON 		0x0F87
#define GOLD 		0x0FD0
...
#if LCD_USING_SPI
#define LCD_PWM_BRT     12
#include <SPI.h>
#define LCD_PHY_SCLK    SCK
#define LCD_PHY_MOSI    MOSI
#define LCD_PHY_RES     8
#define LCD_PHY_CS      7

#define LCD_PIN_RES     5
#define LCD_PORT_RES    *_LCD_PORT_RES

#define LCD_PIN_CS      4
#define LCD_PORT_CS     *_LCD_PORT_CS

#else
#define LCD_PWM_BRT     10
...
#define CHIP_ON(...) XMEM_ACQUIRE_SPI(); cbi(LCD_PORT_CS, LCD_PIN_CS);

#define CHIP_OFF(...) sbi(LCD_PORT_CS, LCD_PIN_CS); XMEM_RELEASE_SPI();
...
private:
        ...
        void setPixelPvt(uint16_t color, unsigned char x, unsigned char y);
        void setCharPvt(char c, int x, int y, uint16_t fColor, uint16_t bColor);
        uint8_t driver;
#if LCD_USING_SPI
        ...
        uint8_t spi_save;
        volatile uint8_t *_LCD_PORT_RES;
        volatile uint8_t *_LCD_PORT_CS;
#endif
public:
        LCDShield();

        void init(bool initspi, int type, bool colorSwap, bool flipdisplay);
        void init(bool initspi, int type = PHILLIPS);
        ...
        void setCircle(int x0, int y0, int radius, uint16_t color, int lineThickness = 1);
        void setArc(int x0, int y0, int radius, int segments[], int numSegments, int lineThickness, uint16_t color);

        void setChar(char c, int x, int y, uint16_t fColor, uint16_t bColor);
        void setStr(char *pString, int x, int y, uint16_t fColor, uint16_t bColor);


        void setLine(int x0, int y0, int x1, int y1, uint16_t color);
        void setRect(int x0, int y0, int x1, int y1, unsigned char fill, uint16_t color);
